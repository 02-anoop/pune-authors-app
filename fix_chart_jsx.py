...
for i, line in enumerate(lines):
    if "<h4 className=\"font-bold text-paa-navy mb-4\">Book Sales Performance Over Time</h4>" in line:
        start_idx = i - 1
    if start_idx != -1 and i > start_idx:
        if "</div>" in line and "</div>" in lines[i-1] and "</ResponsiveContainer>" in lines[i-2]:
            end_idx = i
            break
...
if start_idx != -1 and end_idx != -1:
    ...
    # Now find activeTab === 'performance' and insert it correctly inside it
    perf_idx = -1
    for i, line in enumerate(lines):
        if "{activeTab === 'performance' && (" in line:
            perf_idx = i + 1
            break
            
    if perf_idx != -1:
        # Need to insert right after the opening div of the performance tab block
        for i in range(perf_idx, len(lines)):
            if "<div className=\"space-y-6\">" in lines[i]:
...
"""
                lines.insert(i + 1, chart_ui)
                break

    with open(file_path, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    print("Fixed JSX and added BarChart to correct place!")
else:
    print("Could not find the incorrectly placed chart block!")
