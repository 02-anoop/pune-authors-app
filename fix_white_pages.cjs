...
const startIndex = content.indexOf('export async function loadHtml2Pdf');
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  console.log('Successfully replaced catalogue generation function with true pagination');
} else {
  // If loadHtml2Pdf was replaced already, check for loadPdfLibs
  const startIndex2 = content.indexOf('export async function loadPdfLibs');
  const startIndex3 = content.indexOf('export async function downloadCataloguePDF');
  
  const finalStart = startIndex2 !== -1 ? startIndex2 : (startIndex3 !== -1 ? startIndex3 : -1);
  if (finalStart !== -1 && endIndex !== -1) {
    const before = content.substring(0, finalStart);
    const after = content.substring(endIndex);
    fs.writeFileSync(path, before + replacement + '\n' + after, 'utf8');
    console.log('Successfully replaced catalogue generation function with true pagination (fallback marker)');
  } else {
