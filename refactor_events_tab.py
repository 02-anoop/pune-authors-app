...
new_events_tab = """
  const EventsTab = () => {
    const [eventSubTab, setEventSubTab] = useState<'list' | 'author_data'>('list');
          ...
          <div className="bg-white rounded-xl shadow-premium p-8 border border-gray-200 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <form className="space-y-4" onSubmit={async (e) => {
              ...
              try {
                ...
                // Append aggregate data if no granular data
                if (!hasGranularData) {
              ...
              <div className="border-t border-gray-200 pt-6 mt-6">
                  ...
                  {hasGranularData && (
                      <div className="p-4 bg-blue-50 rounded-xl border border-blue-200 text-sm text-blue-800">
                         * You can manage granular data for each author from the "Author Data Management" tab after creating this event.
                      </div>
                  )}
              </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex border-b border-gray-200 mb-6">
           <button onClick={() => setEventSubTab('list')} className={`px-6 py-4 text-sm font-medium border-b-2 transition-colors ${eventSubTab === 'list' ? 'border-paa-navy text-paa-navy' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>All Events (Tabular)</button>
           <button onClick={() => setEventSubTab('author_data')} className={`px-6 py-4 text-sm font-medium border-b-2 transition-colors ${eventSubTab === 'author_data' ? 'border-paa-navy text-paa-navy' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Author Data Management</button>
        </div>
           ...
           <div className="overflow-x-auto border border-gray-200 rounded-xl bg-white shadow-sm animate-in fade-in duration-500">
             <table className="w-full text-left border-collapse">
               ...
               <tbody className="divide-y divide-gray-100">
                  ...
                  {allCombinedEvents.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-gray-500">No events found.</td></tr>}
               </tbody>
             </table>
           </div>
        )}

        {eventSubTab === 'author_data' && (
           <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 animate-in fade-in slide-in-from-bottom-4 duration-500">
               <div className="flex items-center justify-between mb-6 border-b pb-4">
                 <h3 className="text-2xl font-serif text-paa-navy">Manage Author Event Data</h3>
               </div>
               
               <div className="grid md:grid-cols-2 gap-6 mb-8">
                   <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2">1. Select Event</label>
                      <select 
                         className="w-full border border-gray-300 rounded-lg p-3 outline-none focus:border-paa-navy bg-gray-50"
                         value={selectedEventForData?.id || ''}
                         onChange={(e) => {
                             const evt = allCombinedEvents.find((ev: any) => ev.id.toString() === e.target.value);
                             setSelectedEventForData(evt);
                         }}
                      >
                         <option value="">-- Select Event --</option>
                         {allCombinedEvents.map((evt: any) => (
                            <option key={evt.id} value={evt.id}>{evt.name} {evt.isLegacy ? '(Past/Legacy)' : '(Active)'}</option>
                         ))}
                      </select>
                   </div>
                   
                   <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2">2. Select Author to Edit</label>
                      <select 
                         className="w-full border border-gray-300 rounded-lg p-3 outline-none focus:border-paa-navy bg-gray-50"
                         value={selectedAuthorForData}
                         onChange={(e) => setSelectedAuthorForData(e.target.value)}
                         disabled={!selectedEventForData}
                      >
               ...
               {selectedAuthorForData && selectedEventForData ? (
                  <div className="border border-gray-200 rounded-xl p-6 bg-gray-50 shadow-sm">
                     ...
                     <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end gap-3 items-center">
                        ...
                        <button onClick={() => { setSelectedEventForData(null); setSelectedAuthorForData(''); }} className="px-6 py-2.5 text-sm text-gray-600 hover:bg-gray-200 bg-gray-100 rounded-lg font-bold transition-colors">Reset Selection</button>
                  ...
                  <div className="p-16 text-center border-2 border-dashed border-gray-300 rounded-xl bg-gray-50">
                     ...
                     <p className="text-gray-500 font-medium">Please select an Event and an Author to manage their data.</p>
                  </div>
               )}
           </div>
        )}
