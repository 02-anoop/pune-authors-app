               ...
               {selectedAuthorForData ? (
                  <div className="border border-gray-200 rounded-xl p-6 bg-gray-50 shadow-sm animate-in fade-in slide-in-from-right-4 duration-300">
                     ...
                     <div className="space-y-4">
                        {/* Display a dynamic block for books - using a static example for UI */}
                             ...
                             <tbody className="divide-y divide-gray-100">
                                 {authors.filter((a: any) => a.name.toLowerCase().includes(authorSearch.toLowerCase())).slice(0, 50).map((a: any, i: number) => (
                                     <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                                         ...
                                         <td className="p-3">
                                            <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 text-xs font-bold">Unpublished</span>
                                         </td>
                  ...
                  return (
                      <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="p-4 text-center">
                           <button onClick={() => setSelectedEventBreakdown(evt)} className="text-xs bg-indigo-50 text-indigo-700 hover:bg-indigo-100 px-3 py-1.5 rounded-lg font-bold border border-indigo-200 transition-colors shadow-sm">
                              View Breakdown
                           </button>
                        </td>
                      </tr>
