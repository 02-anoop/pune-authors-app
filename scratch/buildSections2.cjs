...
if (startBuyBooks !== -1 && endBuyBooks !== -1) {
    content = content.substring(0, startBuyBooks) + section1 + '\n\n' + content.substring(endBuyBooks);
}
...
if (stylesIndex !== -1) {
    content = content.substring(0, stylesIndex) + sectionContact + '\n\n  ' + content.substring(stylesIndex);
}

fs.writeFileSync(filePath, content);
console.log("Successfully applied visual layout updates!");
