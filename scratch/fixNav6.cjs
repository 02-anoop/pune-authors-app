...
const replacement = `
      {/* ════════════════════════════════════════════
          BOOKS BY LANGUAGE — PETAL CIRCLES (RESTORED)
      ════════════════════════════════════════════ */}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <FadeIn delay={200}>
            <div style={{ textAlign: "center", marginBottom: "4rem" }}>
              ...
              <p style={{ fontSize: 18, color: "rgba(255,255,255,0.9)", fontWeight: 500 }}>Discover literature in your preferred tongue.</p>
              {languageDrilldown && (
...
// Find the BOOKS BY GENRE & LANGUAGE section and replace it
const startTag = '{/* ════════════════════════════════════════════\n          BOOKS BY GENRE & LANGUAGE\n      ════════════════════════════════════════════ */}';
const startIdx = landContent.indexOf(startTag);

if (startIdx !== -1) {
  // Find the end of this section (the next section is THE BOOK SHOP, CAFÉ & LIBRARY)
  const nextSectionTag = '{/* ════════════════════════════════════════════\n          THE BOOK SHOP, CAFÉ & LIBRARY\n      ════════════════════════════════════════════ */}';
  const endIdx = landContent.indexOf(nextSectionTag, startIdx);
  
  if (endIdx !== -1) {
    landContent = landContent.substring(0, startIdx) + replacement + '\n  ' + landContent.substring(endIdx);
    
    // Also we need to make sure the hero padding was kept correctly.
    // The previous replace for 10rem padding was lost during checkout, let's restore it.
    landContent = landContent.replace(/padding:\s*"4rem 2rem"/, 'padding: "10rem 2rem 4rem 2rem"');

    fs.writeFileSync(landPath, landContent);
    console.log("Restored Petal Circles with Bright Gradient, and restored Hero padding.");
  } else {
    console.log("Could not find the end of the section.");
  }
} else {
  console.log("Could not find BOOKS BY GENRE & LANGUAGE section.");
}
