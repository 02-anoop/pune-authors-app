...
const mergedImports = 'import { ShoppingCart, Search, SlidersHorizontal, Star, ChevronRight, ChevronLeft, ArrowRight, ArrowLeft, X, BookOpen, Info, Download, Loader2, CheckCircle2, Circle } from "lucide-react";';
const mergedButtons = `          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginBottom: "1.5rem" }}>
            <button 
              onClick={() => {
...
const mergedGrid = `              ) : filteredBooks.length === 0 ? (
...
while (i < lines.length) {
  if (lines[i].startsWith('<<<<<<<')) {
    ...
    // Inject the resolved content
    if (conflictCount === 1) resolvedLines.push(mergedImports);
    else if (conflictCount === 2) resolvedLines.push(mergedButtons);
    else if (conflictCount === 3) resolvedLines.push(mergedGrid);
...
console.log("Merged conflicts successfully, properly avoiding line ending issues!");
