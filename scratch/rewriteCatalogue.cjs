...
const newReturn = `  return (
    <main style={{ fontFamily: "var(--font-body)", minHeight: "100vh", background: "#f8f9fa", color: "#111", overflowX: "hidden" }}>
      
      {/* ════════════════════════════════════════════
          CATALOGUE HEADER & HERO
      ════════════════════════════════════════════ */}
      <section style={{ position: "relative", background: "#111", padding: "6rem 2rem 4rem", overflow: "hidden" }}>
        {/* Abstract Blobs */}
        <div style={{ position: "absolute", top: "-50px", left: "-50px", width: "300px", height: "300px", background: "radial-gradient(circle, rgba(255,122,0,0.3) 0%, rgba(0,0,0,0) 70%)", filter: "blur(40px)", pointerEvents: "none" }}></div>
        <div style={{ position: "absolute", bottom: "-100px", right: "10%", width: "400px", height: "400px", background: "radial-gradient(circle, rgba(0,194,255,0.2) 0%, rgba(0,0,0,0) 70%)", filter: "blur(60px)", pointerEvents: "none" }}></div>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "2rem", marginBottom: "3rem" }}>
            <div style={{ maxWidth: 600 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "rgba(255,255,255,0.1)", padding: "0.5rem 1rem", borderRadius: "50px", marginBottom: "1rem", border: "1px solid rgba(255,255,255,0.2)" }}>
                 <BookOpen size={14} color="#00C2FF" />
                 <span style={{ fontSize: 12, fontWeight: 700, color: "#fff", textTransform: "uppercase", letterSpacing: "0.1em" }}>PAA Digital Library</span>
              </div>
              <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.5rem, 5vw, 4rem)", fontWeight: 900, color: "#fff", margin: "0 0 1rem 0", lineHeight: 1.1, letterSpacing: "-0.02em" }}>
                Explore & Buy <span style={{ color: "#00C2FF" }}>Books</span>
              </h1>
              <p style={{ fontSize: 18, color: "rgba(255,255,255,0.6)", margin: 0 }}>Discover incredible titles curated by the Pune Authors' Association.</p>
            </div>

            {totalItems > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: "1rem", background: "#fff", padding: "1rem 2rem", borderRadius: "50px", boxShadow: "0 20px 40px rgba(0,0,0,0.3)", transform: "translateY(10px)" }}>
                ...
                <div>
                   <div style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: "0.1em" }}>Your Cart</div>
          ...
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
             <button 
                ...
                style={{
                  ...
                  background: downloadingType === "standard" ? "#333" : (activeCategory === "All" ? "#00C2FF" : getCategoryColor(activeCategory).color), 
                  ...
                  boxShadow: activeCategory === "All" ? "0 10px 30px rgba(0,194,255,0.4)" : "none",
                }}
      ...
      <section style={{ maxWidth: 1400, margin: "-40px auto 0", padding: "0 2rem", position: "relative", zIndex: 20 }}>
          ...
          {activeCategory === "All" ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "2rem", paddingBottom: "4rem" }}>
              {(Object.keys(bookCategories).filter(cat => allBooks.some(b => b.genre === cat))).map((cat, idx) => {
                ...
                const colors = [
                  { bg: "linear-gradient(135deg, #FF7A00, #FF3D00)", shadow: "rgba(255,122,0,0.4)", icon: "#fff" },
                  { bg: "linear-gradient(135deg, #00C2FF, #0055FF)", shadow: "rgba(0,194,255,0.4)", icon: "#fff" },
                  { bg: "linear-gradient(135deg, #00D084, #008F5D)", shadow: "rgba(0,208,132,0.4)", icon: "#fff" },
                  { bg: "linear-gradient(135deg, #FF4FA3, #C01B6B)", shadow: "rgba(255,79,163,0.4)", icon: "#fff" },
                  { bg: "linear-gradient(135deg, #7B61FF, #4829E6)", shadow: "rgba(123,97,255,0.4)", icon: "#fff" },
                  { bg: "linear-gradient(135deg, #FFD400, #E6A800)", shadow: "rgba(255,212,0,0.4)", icon: "#111" }
                ];
                ...
                return (
                  <div
                    ...
                    onMouseEnter={(e) => {
                      ...
                      if(iconBg) iconBg.style.transform = "scale(1.2) rotate(10deg)";
                    }}
                    onMouseLeave={(e) => {
                  ...
                  >
                    ...
                    <div style={{ position: "absolute", top: -20, right: -20, width: 150, height: 150, borderRadius: "50%", background: "rgba(255,255,255,0.1)", filter: "blur(2px)" }}></div>
                    <div style={{ position: "absolute", bottom: 50, right: -50, width: 100, height: 100, borderRadius: "20%", background: "rgba(255,255,255,0.1)", transform: "rotate(45deg)" }}></div>

                    <div className="cat-icon-bg" style={{ width: 80, height: 80, borderRadius: "24px", background: "rgba(255,255,255,0.2)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "auto", transition: "transform 0.4s" }}>
                      <BookOpen size={40} color={c.icon} />
                    </div>
                    
                    <h3 style={{ margin: "2rem 0 0.5rem 0", color: c.icon, fontSize: "2rem", fontWeight: 900, fontFamily: "var(--font-display)", letterSpacing: "-0.02em", position: "relative", zIndex: 10 }}>{cat}</h3>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "rgba(255,255,255,0.2)", padding: "0.5rem 1rem", borderRadius: "50px", width: "fit-content", backdropFilter: "blur(5px)" }}>
            ...
            <div style={{ background: "#fff", borderRadius: "24px", padding: "1.5rem 2rem", boxShadow: "0 10px 30px rgba(0,0,0,0.05)", marginBottom: "2rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
                ...
                <h2 style={{ fontSize: "2rem", fontWeight: 900, fontFamily: "var(--font-display)", margin: 0, color: getCategoryColor(activeCategory).color }}>{activeCategory}</h2>
              </div>
          ...
          {activeCategory !== "All" && (
            <>
              {isLoading ? (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "2rem", padding: "2rem 0 6rem" }}>
                  {[1,2,3,4,5,6,7,8].map(i => (
                    <div key={i} style={{ background: "#fff", borderRadius: "24px", padding: "1.5rem", height: 400, border: "1px solid #eaeaea", display: "flex", flexDirection: "column" }}>
                      <div style={{ width: "100%", flex: 1, background: "#f5f5f5", borderRadius: "16px", marginBottom: "1rem" }} className="skeleton-pulse"></div>
                ...
                <div style={{ textAlign: "center", padding: "8rem 2rem", background: "#fff", borderRadius: "30px", border: "1px dashed #ccc", marginBottom: "6rem" }}>
                  ...
                  <p style={{ color: "#aaa", fontSize: 15 }}>Try adjusting your filters or search query.</p>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "2.5rem", padding: "2rem 0 6rem" }}>
                  {filteredBooks.map((book) => {
                    const inCart = cart.includes(book.id);
                    return (
                      <div key={book.id} style={{ background: "#fff", borderRadius: "24px", padding: "1.5rem", border: "1px solid #eaeaea", position: "relative", display: "flex", flexDirection: "column", transition: "all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)" }} className="book-card-premium" onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-10px)"; e.currentTarget.style.boxShadow = "0 30px 60px rgba(0,0,0,0.1)"; e.currentTarget.style.borderColor = getCategoryColor(activeCategory).color; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "#eaeaea"; }}>
                        <Link to={\`/book/\${book.id}\`} style={{ textDecoration: "none", flex: 1, display: "flex", flexDirection: "column" }}>
                          <div style={{ width: "100%", aspectRatio: "3/4", borderRadius: "16px", overflow: "hidden", background: "#f5f5f5", marginBottom: "1.5rem", position: "relative", border: "1px solid #eaeaea" }}>
                          ...
                          <h3 style={{ fontSize: 18, fontWeight: 800, color: "#111", margin: "0 0 0.2rem 0", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", fontFamily: "var(--font-display)" }}>{book.title}</h3>
                          <p style={{ fontSize: 14, color: "#888", margin: "0 0 1rem 0", fontWeight: 500 }}>{book.authorName}</p>
                          
                          <div style={{ marginTop: "auto", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                            <div>
                               <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: "#aaa", letterSpacing: "0.1em" }}>Price</div>
                               <div style={{ fontSize: 22, fontWeight: 900, color: "#111" }}>{book.mrp != null ? \`₹\${book.mrp}\` : book.mrpRaw || "TBD"}</div>
                            </div>
                          </div>
                        </Link>
...
console.log("CataloguePage.tsx updated successfully.");
