...
const newReturn = `  return (
    <main style={{ fontFamily: "'Montserrat', sans-serif", background: "#fff", color: "#111", overflowX: "hidden" }}>
      ...
      {/* ════════════════════════════════════════════
          HERO — CLEAN & BRIGHT
      ════════════════════════════════════════════ */}
      <section style={{ position: "relative", minHeight: "80vh", display: "flex", alignItems: "center", overflow: "hidden", background: "#f8f9fa" }}>
        {/* Soft colorful blobs */}
            ...
            <FadeIn>
              ...
              <div style={{ display: "flex", background: "#fff", borderRadius: 50, padding: "0.4rem", border: "1px solid #eaeaea", marginBottom: "2rem", maxWidth: 440, boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
                <input
                  ...
                  style={{ flex: 1, border: "none", outline: "none", background: "transparent", padding: "0 1.5rem", fontSize: 16, color: "#111" }}
                />
                <button
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <FadeIn delay={100}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", marginBottom: "4rem" }}>
              <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 60, height: 60, background: "#fff", borderRadius: 20, marginBottom: "1.5rem", boxShadow: "0 10px 20px rgba(0,0,0,0.1)", transform: "rotate(-10deg)" }}>
              ...
              <p style={{ fontSize: 18, color: "rgba(255,255,255,0.9)", maxWidth: 600, fontWeight: 500 }}>Lose yourself in worlds woven by our finest storytellers.</p>
            </div>
            
            <div style={{ display: "flex", gap: "2rem", overflowX: "auto", paddingBottom: "2rem", alignItems: "flex-start", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
              {galleryItems.filter(b => b.genre === "F").slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} style={{ flex: "0 0 240px", display: "flex", flexDirection: "column", textDecoration: "none", background: "#fff", padding: "1.2rem", borderRadius: "24px", boxShadow: "0 15px 30px rgba(0,0,0,0.1)", transform: "translateY(0)", transition: "transform 0.3s ease, box-shadow 0.3s ease" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-8px)"; e.currentTarget.style.boxShadow = "0 20px 40px rgba(0,0,0,0.15)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 15px 30px rgba(0,0,0,0.1)"; }}>
                  <div style={{ width: "100%", aspectRatio: "3/4", borderRadius: "12px", overflow: "hidden", marginBottom: "1.2rem", background: "#f5f5f5" }}>
                  ...
                  <div style={{ padding: "0 0.5rem" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: "#FF7A00" }}>{book.mrp != null ? \`₹\${book.mrp}\` : book.mrpRaw || "Price TBD"}</div>
              ...
              {galleryItems.filter(b => b.genre === "F").length > 8 && (
                 <div style={{ flex: "0 0 240px", display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
                    <Link to="/catalogue?category=Fiction" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem", color: "#fff", textDecoration: "none", fontWeight: 700 }}>
                       <div style={{ width: 80, height: 80, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.5)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.3s" }} onMouseEnter={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.color = "#FF7A00"; }} onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#fff"; }}>
                         <ArrowRight size={24} />
                       </div>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <FadeIn delay={200}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4rem", flexWrap: "wrap", gap: "2rem" }}>
              <h2 style={{ fontSize: "clamp(2.5rem, 4vw, 3.5rem)", fontWeight: 900, color: "#0033FF", margin: 0, letterSpacing: "-0.03em" }}>Knowledge &<br/>Non-Fiction</h2>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "2rem", alignItems: "stretch" }}>
              {galleryItems.filter(b => b.genre === "NF").slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} style={{ display: "flex", gap: "1.5rem", background: "#fff", padding: "1.5rem", border: "1px solid #eaeaea", borderRadius: 16, textDecoration: "none", transition: "all 0.3s ease" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-5px)"; e.currentTarget.style.boxShadow = "0 15px 30px rgba(0,51,255,0.08)"; e.currentTarget.style.borderColor = "#0033FF"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "#eaeaea"; }}>
                  <div style={{ flex: "0 0 100px", height: "140px", borderRadius: 8, overflow: "hidden", background: "#f5f5f5" }}>
                    <img src={book.coverUrl ? (book.coverUrl.startsWith("http") ? book.coverUrl : \`\${import.meta.env.VITE_API_URL || "http://localhost:3001"}\${book.coverUrl}\`) : "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=300"} alt={book.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  </div>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                    <h4 style={{ fontSize: 16, fontWeight: 800, color: "#111", margin: "0 0 0.4rem 0", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{book.title}</h4>
                    <p style={{ fontSize: 13, color: "#666", margin: "0 0 1rem 0", fontWeight: 500 }}>{book.authorName}</p>
                    <div style={{ fontSize: 16, fontWeight: 800, color: "#0033FF", marginTop: "auto" }}>{book.mrp != null ? \`₹\${book.mrp}\` : book.mrpRaw || "Price TBD"}</div>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section style={{ background: "#FFD400", padding: "6rem 2rem", position: "relative" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <FadeIn delay={300}>
            <div style={{ textAlign: "center", marginBottom: "4rem" }}>
              <div style={{ display: "inline-flex", background: "#00C2FF", padding: "1rem 2.5rem", borderRadius: "50px", transform: "rotate(-2deg)", boxShadow: "5px 5px 0 #111", marginBottom: "1.5rem" }}>
                <h2 style={{ fontSize: "clamp(2rem, 3.5vw, 3rem)", fontWeight: 900, color: "#fff", margin: 0, letterSpacing: "1px", textTransform: "uppercase" }}>Children's Corner</h2>
              </div>
              <p style={{ fontSize: 20, color: "#111", fontWeight: 700 }}>Colorful stories for young, imaginative minds.</p>
            </div>
            
            {/* Added alignItems: "flex-start" to prevent extreme stretching */}
            <div style={{ display: "flex", gap: "2rem", overflowX: "auto", paddingBottom: "2rem", alignItems: "flex-start", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
              {galleryItems.filter(b => b.genre === "C").slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} style={{ flex: "0 0 240px", display: "flex", flexDirection: "column", textDecoration: "none", background: "#fff", borderRadius: "24px", padding: "1.2rem", boxShadow: "6px 6px 0 #00C2FF", transform: "translateY(0) translateX(0)", transition: "all 0.2s ease", border: "2px solid #111" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px) translateX(-4px)"; e.currentTarget.style.boxShadow = "10px 10px 0 #00C2FF"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0) translateX(0)"; e.currentTarget.style.boxShadow = "6px 6px 0 #00C2FF"; }}>
                  {/* Fixed aspect ratio on cover image to avoid weird heights */}
                  <div style={{ width: "100%", aspectRatio: "3/4", borderRadius: "12px", overflow: "hidden", marginBottom: "1.2rem", background: "#f5f5f5", border: "2px solid #111" }}>
                    <img src={book.coverUrl ? (book.coverUrl.startsWith("http") ? book.coverUrl : \`\${import.meta.env.VITE_API_URL || "http://localhost:3001"}\${book.coverUrl}\`) : "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=300"} alt={book.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  </div>
                  <h4 style={{ fontSize: 17, fontWeight: 900, color: "#111", margin: "0 0 0.4rem 0", lineHeight: 1.2, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{book.title}</h4>
                  <p style={{ fontSize: 13, color: "#666", margin: "0 0 1rem 0", fontWeight: 600 }}>{book.authorName}</p>
                  <div style={{ marginTop: "auto", background: "#FF6B6B", color: "#fff", padding: "0.8rem", borderRadius: "12px", textAlign: "center", fontWeight: 800, fontSize: 15, border: "2px solid #111" }}>
      ...
      <section style={{ background: "#fff", padding: "6rem 2rem", position: "relative" }}>
          ...
          <FadeIn>
            ...
            <div style={{ display: "flex", gap: "2rem", overflowX: "auto", paddingBottom: "2rem", alignItems: "flex-start", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
              {[...galleryItems].reverse().slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} className="group" style={{ flex: "0 0 240px", display: "flex", flexDirection: "column", textDecoration: "none", position: "relative" }}>
                  <div style={{ width: "100%", aspectRatio: "3/4", borderRadius: "16px", overflow: "hidden", marginBottom: "1.2rem", background: "#f5f5f5", border: "1px solid #eaeaea", position: "relative" }} className="hover-lift">
                  ...
                  <h4 style={{ fontSize: 16, fontWeight: 800, color: "#111", margin: "0 0 0.4rem 0", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{book.title}</h4>
                  <p style={{ fontSize: 13, color: "#666", margin: "0 0 0.8rem 0", fontWeight: 500 }}>{book.authorName}</p>
                  <div style={{ fontSize: 16, fontWeight: 800, color: "#00D084", marginTop: "auto" }}>{book.mrp != null ? \`₹\${book.mrp}\` : book.mrpRaw || "Price TBD"}</div>
                </Link>
              ))}
            </div>
          </FadeIn>
      ...
      ════════════════════════════════════════════ */}
      <section style={{ background: "#f8f9fa", padding: "6rem 2rem", position: "relative", borderTop: "1px solid #eaeaea" }}>
            ...
            <form onSubmit={async (e) => { e.preventDefault(); setIsSubmitting(true); try { await axios.post(\`\${import.meta.env.VITE_API_URL || "http://localhost:3001"}/api/inquiries\`, { name: contactName, email: contactEmail, message: contactMessage }); toast.success("Inquiry sent successfully!"); setContactName(""); setContactEmail(""); setContactMessage(""); } catch (err) { toast.error("Failed to send inquiry."); } finally { setIsSubmitting(false); } }} style={{ background: "#fff", padding: "3rem", borderRadius: "24px", border: "1px solid #eaeaea", textAlign: "left", boxShadow: "0 20px 40px rgba(0,0,0,0.05)" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "1.5rem", marginBottom: "1.5rem" }}>
                <div>
                  ...
                  <input required value={contactName} onChange={(e) => setContactName(e.target.value)} type="text" style={{ width: "100%", padding: "1rem", background: "#f5f5f5", border: "1px solid #eaeaea", borderRadius: "12px", outline: "none", color: "#111", fontSize: 16, fontWeight: 500 }} onFocus={e => e.currentTarget.style.borderColor = "#00C2FF"} onBlur={e => e.currentTarget.style.borderColor = "#eaeaea"} />
                </div>
                <div>
                  ...
                  <input required value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} type="email" style={{ width: "100%", padding: "1rem", background: "#f5f5f5", border: "1px solid #eaeaea", borderRadius: "12px", outline: "none", color: "#111", fontSize: 16, fontWeight: 500 }} onFocus={e => e.currentTarget.style.borderColor = "#00C2FF"} onBlur={e => e.currentTarget.style.borderColor = "#eaeaea"} />
                </div>
              </div>
...
console.log("LandingPage.tsx updated to bright theme with Montserrat");
