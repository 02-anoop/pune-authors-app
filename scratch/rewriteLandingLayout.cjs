      ...
      <section style={{ background: "#FF7A00", padding: "6rem 2rem", position: "relative" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <FadeIn delay={100}>
            ...
            <div style={{ display: "flex", gap: "2rem", overflowX: "auto", paddingBottom: "2rem", alignItems: "stretch", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
              {galleryItems.filter(b => b.genre === "F").slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} style={{ flex: "0 0 300px", display: "flex", flexDirection: "column", textDecoration: "none", background: "#fff", padding: "1.2rem", borderRadius: "24px", boxShadow: "0 15px 30px rgba(0,0,0,0.1)", transform: "translateY(0)", transition: "transform 0.3s ease, box-shadow 0.3s ease" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-8px)"; e.currentTarget.style.boxShadow = "0 20px 40px rgba(0,0,0,0.15)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 15px 30px rgba(0,0,0,0.1)"; }}>
                  ...
                  <div style={{ padding: "0 0.5rem", display: "flex", flexDirection: "column", flex: 1 }}>
                    ...
                    <p style={{ fontSize: 14, color: "#666", margin: "0 0 1rem 0", fontWeight: 500 }}>{book.authorName}</p>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto" }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: "#FF7A00" }}>{book.mrp != null ? \`₹\${book.mrp}\` : book.mrpRaw || "Price TBD"}</div>
          ...
          <FadeIn delay={200}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "2rem", marginBottom: "4rem" }}>
              <div>
                 ...
                 <p style={{ fontSize: 18, color: "#666", maxWidth: 600, fontWeight: 500, margin: 0 }}>Explore real-world insights, histories, and thought-provoking analysis.</p>
              </div>
              ...
              {galleryItems.filter(b => b.genre === "NF").slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} style={{ display: "flex", gap: "1.5rem", background: "#fff", padding: "1.5rem", border: "1px solid #eaeaea", borderRadius: 16, textDecoration: "none", transition: "all 0.3s ease" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-5px)"; e.currentTarget.style.boxShadow = "0 15px 30px rgba(0,51,255,0.08)"; e.currentTarget.style.borderColor = "#0033FF"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "#eaeaea"; }}>
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                    <h4 style={{ fontSize: 17, fontWeight: 800, color: "#111", margin: "0 0 0.4rem 0", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden", fontFamily: "'Playfair Display', serif" }}>{book.title}</h4>
                    <p style={{ fontSize: 14, color: "#666", margin: "0 0 1rem 0", fontWeight: 500 }}>{book.authorName}</p>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4rem", flexWrap: "wrap", gap: "2rem" }}>
              <div style={{ textAlign: "left" }}>
                <div style={{ display: "inline-flex", background: "#00C2FF", padding: "1rem 2.5rem", borderRadius: "50px", transform: "rotate(-2deg)", boxShadow: "5px 5px 0 #111", marginBottom: "1.5rem" }}>
                  <h2 style={{ fontSize: "clamp(2rem, 3.5vw, 3rem)", fontWeight: 900, color: "#fff", margin: 0, letterSpacing: "1px", textTransform: "uppercase", fontFamily: "'Google Sans', sans-serif !important" }}>Children's Corner</h2>
                </div>
                <p style={{ fontSize: 20, color: "#111", fontWeight: 700, margin: 0 }}>Colorful stories for young, imaginative minds.</p>
              </div>
              <Link to="/catalogue?category=Children" style={{ background: "#111", color: "#fff", padding: "1rem 2.5rem", borderRadius: 50, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", textDecoration: "none", fontSize: 13, transition: "transform 0.3s" }} onMouseEnter={e => e.currentTarget.style.transform = "translateY(-3px)"} onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}>
                View All
              </Link>
            </div>
            
            {/* Horizontal, playful cards (bigger width, shorter height) */}
            <div style={{ display: "flex", gap: "2rem", overflowX: "auto", paddingBottom: "2rem", alignItems: "stretch", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
              {galleryItems.filter(b => b.genre === "C").slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} style={{ flex: "0 0 380px", display: "flex", flexDirection: "row", gap: "1.5rem", textDecoration: "none", background: "#fff", borderRadius: "24px", padding: "1.2rem", boxShadow: "8px 8px 0 #00C2FF", transform: "translateY(0) translateX(0)", transition: "all 0.2s ease", border: "3px solid #111", alignItems: "center" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px) translateX(-4px)"; e.currentTarget.style.boxShadow = "12px 12px 0 #00C2FF"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0) translateX(0)"; e.currentTarget.style.boxShadow = "8px 8px 0 #00C2FF"; }}>
                  <div style={{ flex: "0 0 120px", aspectRatio: "3/4", borderRadius: "12px", overflow: "hidden", background: "#f5f5f5", border: "2px solid #111" }}>
                    <img src={book.coverUrl ? (book.coverUrl.startsWith("http") ? book.coverUrl : \`\${import.meta.env.VITE_API_URL || "http://localhost:3001"}\${book.coverUrl}\`) : "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=300"} alt={book.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  </div>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", justifyContent: "center" }}>
                    ...
                    {/* Placeholder for ratings since they might exist */}
                    ...
                    <div style={{ background: "#FF6B6B", color: "#fff", padding: "0.6rem 1rem", borderRadius: "50px", textAlign: "center", fontWeight: 900, fontSize: 16, border: "2px solid #111", width: "fit-content" }}>
          ...
          <FadeIn>
            ...
            <div style={{ display: "flex", gap: "2rem", overflowX: "auto", paddingBottom: "2rem", alignItems: "stretch", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
              {[...galleryItems].reverse().slice(0, 8).map((book, i) => (
                <Link key={i} to={\`/book/\${book.id}\`} className="group" style={{ flex: "0 0 280px", display: "flex", flexDirection: "column", textDecoration: "none", position: "relative" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
                    <h4 style={{ fontSize: 18, fontWeight: 800, color: "#111", margin: "0 0 0.4rem 0", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", fontFamily: "'Playfair Display', serif" }}>{book.title}</h4>
                    <p style={{ fontSize: 14, color: "#666", margin: "0 0 0.8rem 0", fontWeight: 500 }}>{book.authorName}</p>
                    <div style={{ fontSize: 20, fontWeight: 800, color: "#00D084", marginTop: "auto" }}>{book.mrp != null ? \`₹\${book.mrp}\` : book.mrpRaw || "Price TBD"}</div>
                  </div>
                </Link>
              ))}
            </div>
...
console.log("LandingPage.tsx updated with new layout constraints");
