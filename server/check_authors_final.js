...
async function check() {
  ...
  for (const author of authors) {
    ...
    if (participatedCount !== validParticipations.length) {
      mismatchedAuthors.push(`${author.name} (Backend: ${participatedCount}, Frontend: ${validParticipations.length})`);
    } else {
      console.log(`${author.name}: MATCH (${participatedCount})`);
    }
  }
