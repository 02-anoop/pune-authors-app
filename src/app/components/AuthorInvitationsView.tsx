...
const formatInvitationDate = (dateStr: string) => {
  ...
  try {
    // If format is YYYY-MM-DD, parsing with T00:00:00 ensures local timezone parsing
...
export const AuthorInvitationsView = () => {
  ...
  const filteredInvitations = invitations.filter((inv) => {
    ...
    if (statusFilter === 'Pending') {
      matchesStatus = inv.status === 'Sent to Author';
    } else if (statusFilter === 'Accepted') {
      matchesStatus = inv.status === 'Accepted by Author';
    } else if (statusFilter === 'Declined') {
      matchesStatus = inv.status === 'Rejected by Author';
    }
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      {/* Page Header */}
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-paa-navy/5 shadow-sm">
        <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white p-4 rounded-xl border border-paa-navy/5 shadow-sm">
        {/* Search Input */}
      ...
      {/* Table Container */}
      <div className="bg-white rounded-2xl border border-paa-navy/5 shadow-sm overflow-hidden">
                ...
                filteredInvitations.map((inv, idx) => (
                  <tr key={inv.id} className={`transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-[#ebd8c0]'} hover:bg-indigo-50/20`}>
                    ...
                    <td className="px-6 py-4 truncate">
                      <div className="font-bold text-paa-navy text-sm truncate" title={inv.eventTitle}>
                        {inv.eventTitle}
                      </div>
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-slate-400 font-medium mt-0.5 ml-5">
                        {inv.eventTime}
                      </div>
                    </td>
                    ...
                    {/* Organizer */}
                    <td className="px-6 py-4 truncate">
                      <div className="font-bold text-xs text-slate-800 truncate" title={inv.customerName}>
                        {inv.customerName}
                      </div>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`text-[9px] font-black uppercase tracking-wider px-2.5 py-1.5 rounded-full text-white shadow-sm ${
                        inv.status === 'Sent to Author' ? 'bg-amber-500' :
                        inv.status === 'Accepted by Author' ? 'bg-emerald-600' :
                        inv.status === 'Rejected by Author' ? 'bg-rose-500' :
                        'bg-slate-500'
                      }`}>
                        {inv.status === 'Sent to Author' ? 'Pending' : inv.status.split(' ')[0]}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="p-1.5 bg-[#2a4b6b] text-white hover:bg-[#1f374e] rounded-lg transition-colors flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider shadow-sm border border-transparent"
                          title="View Details"
                        >
                          <Eye size={13} />
                          <span>Details</span>
      ...
      {/* View Details Modal */}
      {selectedInvitation && (
        <div className="fixed inset-0 bg-paa-navy/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl border border-paa-navy/5 max-w-2xl w-full max-h-[90vh] overflow-y-auto relative animate-in zoom-in-95 duration-250 flex flex-col">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`text-[9px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full ${
                    selectedInvitation.status === 'Sent to Author' ? 'bg-amber-100 text-amber-800 border border-amber-200' :
                    selectedInvitation.status === 'Accepted by Author' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                    selectedInvitation.status === 'Rejected by Author' ? 'bg-rose-100 text-rose-800 border border-rose-200' :
                ...
                <h3 className="text-lg font-bold font-serif text-paa-navy leading-snug">{selectedInvitation.eventTitle}</h3>
              </div>
              <button 
                onClick={() => setSelectedInvitation(null)} 
                className="text-gray-400 hover:text-gray-600 transition-colors p-1.5 hover:bg-gray-50 rounded-lg border border-gray-100"
              >
                <X size={16} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-150">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-700">{formatInvitationDate(selectedInvitation.eventDate)} ({selectedInvitation.eventTime})</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-700 truncate max-w-[150px]" title={selectedInvitation.venue}>
                      {selectedInvitation.venue}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
              ...
              {selectedInvitation.adminRemarks && (
                <div className="bg-amber-50 p-4 rounded-xl border border-amber-150 flex gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-905 leading-relaxed">{selectedInvitation.adminRemarks}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Modal Action Footer */}
            <div className="p-5 bg-slate-50 border-t border-gray-100 flex justify-end gap-3 shrink-0 rounded-b-2xl">
              {selectedInvitation.status === 'Sent to Author' ? (
                <>
                  <button 
                    onClick={() => {
                      respondToInvitation(selectedInvitation.id, 'Rejected by Author');
                      setSelectedInvitation(null);
                    }}
                    className="px-4 py-2 bg-white border border-slate-200 text-rose-600 hover:bg-rose-50 text-[11px] font-bold uppercase tracking-wider rounded-xl transition-all duration-200 flex items-center gap-1.5 shadow-sm"
                  >
                    <XCircle size={15} /> Decline
                  </button>
                  <button 
                    onClick={() => {
                      respondToInvitation(selectedInvitation.id, 'Accepted by Author');
                      setSelectedInvitation(null);
                    }}
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold uppercase tracking-wider rounded-xl transition-all duration-200 flex items-center gap-1.5 shadow-sm hover:shadow"
                  >
