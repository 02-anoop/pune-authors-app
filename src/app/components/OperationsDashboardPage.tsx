...
import {
  ...
  LayoutDashboard, LayoutGrid, CheckCircle, Clock, ChevronRight, ChevronLeft, Download, BarChart2, DollarSign, ExternalLink, HelpCircle, Key, Globe, Mail, PieChart, Activity, Printer, FileDown, CheckSquare, Lock, MessageSquare, Star, Megaphone, UserCircle, Send, User, Phone, Library, Upload, Truck, PackageCheck
} from 'lucide-react';
import {
...
const SettingsTabComponent = ({ books, API }: { books: any[], API: string }) => {
      ...
      <div className="space-y-8 w-full animate-fade-in-up">
        ...
        <div className="bg-white p-8 border border-paa-navy/5 shadow-premium hover:shadow-premium-hover hover:-translate-y-1 transition-all duration-500 ease-out rounded-3xl-2xl">
          <div className="border-b border-paa-navy/5 pb-4 mb-8">
            <h2 className="text-xl font-serif font-medium text-paa-navy mb-1">System Overrides</h2>
        ...
        <div className="bg-white p-8 border border-paa-navy/5 shadow-premium hover:shadow-premium-hover hover:-translate-y-1 transition-all duration-500 ease-out rounded-3xl-2xl">
          <div className="border-b border-paa-navy/5 pb-4 mb-8">
            <h2 className="text-xl font-serif font-medium text-paa-navy mb-1">Main Landing Page Content</h2>
        ...
        <div className="bg-white p-8 border border-paa-navy/5 shadow-premium hover:shadow-premium-hover hover:-translate-y-1 transition-all duration-500 ease-out rounded-3xl-2xl">
          <div className="border-b border-paa-navy/5 pb-4 mb-8">
            <h2 className="text-xl font-serif font-medium text-paa-navy mb-1">Author Landing Page Content</h2>
            <p className="text-paa-gray-text text-sm">Configure the hero text for the /authors page.</p>
          </div>
          <div className="space-y-6">
            <div>
              <label className="block text-xs font-bold tracking-widest uppercase text-paa-navy mb-2">Hero Title</label>
              <input
                ...
                onChange={(e) => setSettings({ ...settings, author_hero_title: e.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, author_hero_highlight: e.target.value })}
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setSettings({ ...settings, author_hero_subtitle: e.target.value })}
        ...
        <div className="bg-white p-8 border border-paa-navy/5 shadow-premium hover:shadow-premium-hover hover:-translate-y-1 transition-all duration-500 ease-out rounded-3xl-2xl">
          <div className="border-b border-paa-navy/5 pb-4 mb-8">
            <h2 className="text-xl font-serif font-medium text-paa-navy mb-1">Organizer Landing Page Content</h2>
            <p className="text-paa-gray-text text-sm">Configure the hero text for the /organizers page.</p>
          </div>
          <div className="space-y-6">
            <div>
              <label className="block text-xs font-bold tracking-widest uppercase text-paa-navy mb-2">Hero Title</label>
              <input
                ...
                onChange={(e) => setSettings({ ...settings, organizer_hero_title: e.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, organizer_hero_highlight: e.target.value })}
                placeholder="e.g. Authors"
                className="w-full border border-paa-navy/20 bg-gray-50 rounded-lg p-3 text-sm outline-none focus:border-paa-navy focus:bg-white transition-colors"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setSettings({ ...settings, organizer_hero_subtitle: e.target.value })}
...
export const getAuthorParticipationStats = (author: any, allEvents: any[]) => {
  ...
  const participatedCount = (author.eventAuthors || []).filter((ea: any) => 
    ea.optInStatus !== 'Pending' && ea.optInStatus !== 'Rejected' && eligibleEventIds.includes(ea.eventId)
  ).length;

  const total = eligibleEventIds.length;
  const percentage = total === 0 ? 0 : Math.round((participatedCount / total) * 100);
...
export function OperationsDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'orders' | 'web_orders' | 'sales_report' | 'authors' | 'books' | 'inventory' | 'events' | 'forms' | 'gallery' | 'reviews' | 'invitations' | 'late_authors' | 'helpdesk' | 'settings' | 'library_donations' | 'broadcasts' | 'event-requests' | 'documents'>(
  ...
  const [pendingAlerts, setPendingAlerts] = useState({ orders: false, queries: false, authors: false, books: false, fines: false });
  ...
  const prevCountsRef = React.useRef({ orders: 0, queries: 0, authors: 0, books: 0, fines: 0 });
  ...
  const [rankingMode, setRankingMode] = useState<'participation' | 'books'>('participation');
  ...
  const [notificationDocument, setNotificationDocument] = useState<File | null>(null);
  ...
  const handleRejectEventSubmit = async () => {
    ...
    try {
      await axios.post(`${API}/api/admin/events/${rejectEventTarget.eventId}/author/${rejectEventTarget.authorId}/reject`, { reason }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const fetchServerFiles = async () => {
    try {
      const res = await axios.get(`${API}/api/admin/server-files`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  const fetchAuthors = async (isBackground = false) => {
    ...
    try {
      ...
      const pendingFineCount = latestAuthors.filter((a: any) => { const ed = typeof a.extraData === 'string' ? (() => { try { return JSON.parse(a.extraData) } catch(e) { return {} } })() : (a.extraData || {}); return ed?.fineStatus === 'Pending Verification' || (!ed?.fineStatus && ed?.finePaymentScreenshot); }).length;
      if (pendingFineCount > prevCountsRef.current.fines) setPendingAlerts(prev => ({ ...prev, fines: true }));
  ...
  const fetchEvents = async (isBackground = false) => {
    if (!isBackground) setIsRefreshing(true);
    try {
      const res = await axios.get(`${API}/api/admin/events?_t=${new Date().getTime()}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
      let allEvts = res.data;
      try {
        const propRes = await axios.get(`${API}/api/admin/proposed-events`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
        allEvts = [...propRes.data, ...res.data];
      } catch (err) {}
      setEvents(allEvts); sessionStorage.setItem('adminEvents', JSON.stringify(res.data));
    } catch (err) { } finally { if (!isBackground) setIsRefreshing(false); }
  };
  ...
  const handleDeleteServerFile = async (filename: string) => {
    if (window.confirm('Are you sure you want to delete this uploaded doc?')) {
      await axios.delete(`${API}/api/admin/server-files/${filename}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  useEffect(() => {
    ...
    const checkAndGenerateCatalogue = async () => {
      try {
        const pdfUrl = API ? `${API}/uploads/catalogue.pdf` : 'http://localhost:3001/uploads/catalogue.pdf';
        const res = await fetch(pdfUrl, { method: 'HEAD' });
        if (!res.ok && res.status === 404) {
          console.log("Catalogue PDF not found on server, generating first-time backup silently...");
  ...
  const autoRegenerateCompleteCatalogue = async () => {
    try {
      ...
      await downloadCataloguePDF('Complete', formattedBooks, () => { }, {}, false, true, true);
  ...
  const handleSendNotification = async (e?: React.FormEvent, forceAll?: boolean) => {
    if (e) e.preventDefault();
  ...
  const handleDeleteNotification = async (id: number) => {
    if (!window.confirm('Are you sure you want to delete this document/notification? This action cannot be undone.')) return;
    try {
      const token = localStorage.getItem('token');
              ...
              <div className="bg-white p-6 rounded-2xl border border-paa-navy/5 shadow-sm">
                ...
                <div className="h-48 w-full">
                  {revenueTrendData.length > 0 ? (
                    ...
                    <div className="h-full flex items-center justify-center text-gray-500 text-xs">No revenue data.</div>
                  )}
                </div>
              </div>
                        ...
                        <td colSpan={6} className="p-0">
                          <div className="p-6 md:p-8 animate-fade-in-up">
                            <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                              {book.coverUrl && (
<<<<<<< Updated upstream
                                <img loading="lazy" src={book.coverUrl.startsWith('http') ? book.coverUrl : `${API}${book.coverUrl}`} alt="Cover" className="w-40 h-56 object-cover border border-paa-navy/20 shadow-md rounded" />
=======
...
<<<<<<< Updated upstream
                              <img loading="lazy" src={generatedPosterPreview} alt="Generated Poster" className="w-full max-w-[150px] h-auto object-contain border-2 border-white shadow-sm rounded hover:shadow-md transition-shadow cursor-pointer" />
=======
...
<<<<<<< Updated upstream
                      <img loading="lazy" src="/logo.png" alt="PAA Logo" className="max-w-full max-h-full object-contain" crossOrigin="anonymous" />
=======
              ...
              {!selectedAuthorForData && (
                <>
                  ...
                  <button onClick={handleDownloadEventReport} className="dash-btn bg-[#ebd8c0] text-emerald-700 hover:bg-[#ebd8c0] border border-emerald-200 transition-colors shadow-sm font-bold flex items-center gap-2">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><title>Download Report</title><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg> Download Report
                  </button>
                </>
              )}
              <button onClick={() => {
...
<<<<<<< Updated upstream
                                            <img loading="lazy" src={`${a.paymentScreenshot.startsWith('http') ? a.paymentScreenshot : API + a.paymentScreenshot}`} className="w-full h-full object-cover" alt="Proof" />
=======
...
<<<<<<< Updated upstream
                                    <img loading="lazy" src={evt.bannerUrl.startsWith('http') ? evt.bannerUrl : `${API}${evt.bannerUrl}`} alt="Banner" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
=======
          ...
          <div className="dash-panel animate-fade-in-up">
            <div className="dash-panel-header flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
              <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                <button onClick={() => setGallerySubTab('events')} className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-xl transition-colors text-center whitespace-nowrap ${gallerySubTab === 'events' ? 'bg-paa-navy text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:text-paa-navy hover:bg-gray-200'}`}>Event Galleries</button>
                <button onClick={() => setGallerySubTab('carousel')} className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-xl transition-colors text-center whitespace-nowrap ${gallerySubTab === 'carousel' ? 'bg-paa-navy text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:text-paa-navy hover:bg-gray-200'}`}>Featured Gallery Images</button>
              </div>
...
<<<<<<< Updated upstream
                        <img loading="lazy" src={`${API}${img.url}`} alt="Featured Image" className="w-full h-full object-cover" />
=======
                        <img src={`${API}${img.url}`} alt="Featured Image" className="w-full h-full object-cover"   />
>>>>>>> Stashed changes
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3">
                          <div className="flex items-center gap-3">
                            <button onClick={() => handleCarouselMove(index, 'left')} disabled={index === 0} className="w-8 h-8 bg-black/50 text-white rounded-full flex items-center justify-center hover:bg-black/80 transition-colors disabled:opacity-30 disabled:cursor-not-allowed shadow-md" title="Move Left">
                              <ChevronLeft size={16} />
                            </button>
                            ...
                            <button onClick={() => handleCarouselMove(index, 'right')} disabled={index === carouselImages.length - 1} className="w-8 h-8 bg-black/50 text-white rounded-full flex items-center justify-center hover:bg-black/80 transition-colors disabled:opacity-30 disabled:cursor-not-allowed shadow-md" title="Move Right">
                ...
                <>
                  <div className="flex flex-col md:flex-row gap-4 mb-6 bg-white p-4 rounded-xl border border-paa-navy/5 shadow-sm">
                    <div className="flex-1 relative w-full min-w-[200px]">
                      ...
                      <input type="text" placeholder="Search by name or location..." value={galleryTabSearchTerm} onChange={e => setGalleryTabSearchTerm(e.target.value)} className="dash-input w-full" style={{ paddingLeft: '2.5rem' }} />
                    </div>
                    <select value={galleryTabFilterType} onChange={e => setGalleryTabFilterType(e.target.value)} className="dash-input !w-full md:!w-48 shrink-0">
                      <option value="">All Event Types</option>
                      <option value="Book Fair">Book Fair</option>
                      <option value="Literary Event">Literary Event</option>
                      <option value="Airport Library">Flybraries</option>
                    </select>
                    <input type="date" value={galleryTabFilterDate} onChange={e => setGalleryTabFilterDate(e.target.value)} className="dash-input !w-full md:!w-40 shrink-0" />
                    <select value={galleryTabSortBy} onChange={e => setGalleryTabSortBy(e.target.value)} className="dash-input !w-full md:!w-48 shrink-0">
                      ...
                      return (
                        <div key={evt.id} className="group flex flex-col bg-white border border-paa-navy/10 rounded-xl overflow-hidden hover:shadow-premium transition-all duration-300">
                          <div className="relative h-48 w-full overflow-hidden bg-paa-navy/5 shrink-0">
                            {bannerUrl ? (
<<<<<<< Updated upstream
                              <img loading="lazy" src={bannerUrl} alt={evt.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-in-out" />
=======
...
<<<<<<< Updated upstream
                          <img loading="lazy" src={URL.createObjectURL(bannerUploadFile)} className="w-full h-full object-cover" alt="Banner Preview" />
=======
...
>>>>>>> Stashed changes
                      ...
                      {selectedGalleryEvent.bannerUrl && !bannerUploadFile && (
                        <div className="mt-2">
                          ...
                          <div className="relative">
<<<<<<< Updated upstream
                            <img loading="lazy" src={`${API}${selectedGalleryEvent.bannerUrl}`} className="w-full aspect-video object-cover rounded-lg border border-gray-200" alt="Current Banner" />
=======
...
>>>>>>> Stashed changes
                            <button type="button" onClick={handleDeleteBanner} disabled={isDeletingBanner} className="absolute top-2 right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 shadow-sm transition-all disabled:opacity-50" title="Delete Banner">
                              <Trash2 size={12} />
                            </button>
                          </div>
                        </div>
...
<<<<<<< Updated upstream
                      <img loading="lazy" src={`${API}${img.url}`} className="w-full h-full object-cover" alt="Gallery photo" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
=======
                      <img src={`${API}${img.url}`} className="w-full h-full object-cover" alt="Gallery photo" onError={(e) => { e.currentTarget.style.display = 'none'; }} loading="lazy" decoding="async" />
>>>>>>> Stashed changes
      ...
      <aside className={`w-64 flex flex-col shrink-0 h-screen fixed md:sticky top-0 bg-paa-cream z-50 transform transition-transform duration-300 border-r border-paa-navy/5 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-4 md:p-6 h-20 flex items-center justify-between shrink-0 border-b border-paa-navy/5">
          <div className="flex items-center gap-2">
            <img loading="lazy"
              ...
              onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling?.classList.remove('hidden'); }} loading="eager" decoding="async" width={120} height={24}
            />
        ...
        <nav className="flex-1 py-5 px-4 space-y-1.5 overflow-y-auto">
          {[
            ...
            { id: 'late_authors', label: 'Late Authors System', icon: AlertCircle, hasAlert: pendingAlerts.fines },
            { id: 'helpdesk', label: 'Helpdesk / Queries', icon: Users, hasAlert: pendingAlerts.queries },
            { id: 'event-requests', label: 'Organize Event', icon: CalendarIcon },
            { id: 'invitations', label: 'Author Invitations', icon: CalendarIcon },
...
<<<<<<< Updated upstream
            <img loading="lazy" src="/logo.webp" alt="PAA Logo" width="100" height="24" className="h-6 w-auto object-contain md:hidden mr-1" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling?.classList.remove('hidden'); }} />
=======
            <img src="/logo.webp" alt="PAA Logo" className="h-6 w-auto object-contain md:hidden mr-1" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling?.classList.remove('hidden'); }} loading="eager" decoding="async" width={120} height={24} />
>>>>>>> Stashed changes
          ...
          <div className="flex items-center gap-3 relative">
            ...
            >
              ...
              {(pendingAlerts.orders || pendingAlerts.queries || pendingAlerts.authors || pendingAlerts.books || pendingAlerts.fines) && (
              ...
              <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, width: 420, background: '#fff', borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', zIndex: 9999, overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(0,0,0,0.06)', background: '#fafafa' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <button aria-label="Close Notifications" onClick={() => setShowNotifications(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}><X className="w-4 h-4" /></button>
                  </div>
                ...
                <div style={{ padding: '16px 20px', position: 'relative' }}>
                  ...
                  {showMentionDropdown && (
                    <div style={{ position: 'absolute', left: 20, right: 20, bottom: '100%', marginBottom: -60, background: '#fff', border: '1px solid rgba(0,0,0,0.12)', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', maxHeight: 180, overflowY: 'auto', zIndex: 10 }}>
                      {authors
                        .filter((a: any) => (a.name || '').toLowerCase().includes(mentionQuery.toLowerCase()) || (a.email || '').toLowerCase().includes(mentionQuery.toLowerCase()))
                        .slice(0, 8)
                        .map((a: any) => (
                          <button
                            key={a.id}
                            onClick={() => handleMentionSelect(a.name || a.email || 'Author')}
                          ...
                          >
                            ...
                            <div>
                              <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a2e' }}>{a.name || a.email}</div>
                              {a.name && <div style={{ fontSize: 11, color: '#6b6b80' }}>{a.email || a.phone || 'Author'}</div>}
                            </div>
                          </button>
                        ))
                      }
                      {authors.filter((a: any) => (a.name || '').toLowerCase().includes(mentionQuery.toLowerCase()) || (a.email || '').toLowerCase().includes(mentionQuery.toLowerCase())).length === 0 && (
                ...
                {notifications.filter((n: any) => !n.documentUrl).length > 0 && (
                  <div style={{ borderTop: '1px solid rgba(0,0,0,0.06)', maxHeight: 160, overflowY: 'auto' }}>
                    <div style={{ padding: '10px 20px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#6b6b80' }}>Recent Broadcasts</span>
                      <button onClick={() => { setShowNotifications(false); setActiveTab('broadcasts' as any); localStorage.setItem('adminActiveTab', 'broadcasts'); }} style={{ background: 'transparent', border: 'none', color: '#3b82f6', fontSize: 10, fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase' }}>
                        View All
                      </button>
                    </div>
                    {notifications.filter((n: any) => !n.documentUrl).slice(0, 5).map((n: any) => (
        ...
        <div id="admin-dashboard-scroll" className="flex-1 overflow-auto p-4 sm:p-7">
          <Suspense fallback={<div className="p-10 text-center text-gray-500 font-medium">Loading module...</div>}>
            {activeTab === 'overview' && <React.Suspense fallback={<div className="p-8 text-center text-gray-500 animate-pulse font-medium">Loading Dashboard Overview...</div>}>
                    <AdminOverviewTab refreshTrigger={lastRefreshTime} books={books} authors={authors} orders={orders} events={events} stats={stats} prevQueries={prevCountsRef.current?.queries || 0} API={API} setActiveTab={setActiveTab} setAuthorStatusFilter={setAuthorStatusFilter} />
                  </React.Suspense>}
            {activeTab === 'broadcasts' && (
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm animate-fade-in-up">
                <h2 className="text-xl font-serif text-paa-navy mb-6 flex items-center gap-2"><Megaphone className="w-5 h-5 text-indigo-500"/> All Broadcasted Messages</h2>
                {notifications.filter((n: any) => !n.documentUrl).length === 0 ? (
                  <p className="text-gray-500 italic text-sm">No broadcasts found.</p>
                ) : (
                  <div className="space-y-4">
                    {notifications.filter((n: any) => !n.documentUrl).map((n: any) => (
                      <div key={n.id} className="p-5 bg-[#f8fafc] border border-gray-100 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-shadow">
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-paa-navy mb-3 leading-relaxed">{n.message}</p>
                          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                            <span className="font-bold text-[10px] uppercase tracking-widest bg-white border border-gray-200 px-2 py-1 rounded-md">Target: {n.target === 'ALL' ? 'All Authors' : `@${n.target}`}</span>
                            <span className="font-medium flex items-center gap-1"><CalendarIcon className="w-3 h-3"/> {new Date(n.createdAt).toLocaleDateString()} at {new Date(n.createdAt).toLocaleTimeString()}</span>
                          </div>
                        </div>
                        <button onClick={() => handleDeleteNotification(n.id)} className="p-2.5 text-red-500 bg-white border border-red-100 hover:bg-red-50 rounded-xl transition-colors shadow-sm shrink-0" title="Delete Broadcast">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
            ...
            {activeTab === 'documents' && (
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm animate-fade-in-up">
                <h2 className="text-xl font-serif text-paa-navy mb-6 flex items-center gap-2"><FileText className="w-5 h-5 text-indigo-500"/> Admin Documents</h2>
                
                <div className="bg-[#f8fafc] border border-gray-100 rounded-2xl p-5 mb-8">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-paa-navy mb-4">Upload & Share Document</h3>
                  <div className="flex flex-col gap-4">
                    <div>
                      <label className="dash-label">Document Message / Description</label>
                      <input 
                    ...
                    <div>
                      <label className="dash-label">Attach File (Required)</label>
                      <input 
                        type="file" 
                        className="dash-input text-xs w-full bg-white" 
                        onChange={(e) => setNotificationDocument(e.target.files ? e.target.files[0] : null)}
                      />
                    </div>
                ...
                <h3 className="text-sm font-bold uppercase tracking-widest text-paa-navy mb-4">Previously Sent Documents</h3>
                {notifications.filter((n: any) => n.documentUrl).length === 0 ? (
                  <p className="text-gray-500 italic text-sm">No documents have been shared yet.</p>
                ) : (
                  <div className="space-y-4">
                    {notifications.filter((n: any) => n.documentUrl).map((n: any) => (
                      <div key={n.id} className="p-5 bg-white border border-gray-100 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-shadow">
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-paa-navy mb-3 leading-relaxed">{n.message || 'No description'}</p>
                          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                            <span className="font-medium flex items-center gap-1"><CalendarIcon className="w-3 h-3"/> {new Date(n.createdAt).toLocaleDateString()}</span>
                            <a href={`${API}${n.documentUrl}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-700 font-bold uppercase tracking-widest text-[10px] rounded-md hover:bg-indigo-100 transition-colors">
                              <FileText className="w-3 h-3" /> {n.documentName || 'View Document'}
                            </a>
                          </div>
                        </div>
                        <button onClick={() => handleDeleteNotification(n.id)} className="p-2.5 text-red-500 bg-white border border-red-100 hover:bg-red-50 rounded-xl transition-colors shadow-sm shrink-0" title="Delete Document">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                ...
                <div className="mt-12 pt-8 border-t border-gray-100">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-paa-navy mb-4">All Uploaded Docs (Server)</h3>
                  <p className="text-xs text-gray-500 mb-6">These are raw documents stored in the server's uploads folder (e.g. Catalogue PDF, internal forms).</p>
                  {serverFiles.length === 0 ? (
                    <p className="text-gray-500 italic text-sm">No raw documents found on the server.</p>
                  ) : (
                    <div className="space-y-4">
                      {serverFiles.map((file: any, idx: number) => (
                        <div key={idx} className="p-5 bg-gray-50 border border-gray-200 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-shadow">
                          <div className="flex-1">
                            <p className="text-sm font-semibold text-paa-navy mb-3 leading-relaxed">{file.name}</p>
                            <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                              <span className="font-medium flex items-center gap-1"><CalendarIcon className="w-3 h-3"/> {new Date(file.createdAt).toLocaleDateString()}</span>
                              <span className="font-medium bg-gray-200 px-2 py-0.5 rounded text-[10px]">{(file.size / 1024).toFixed(1)} KB</span>
                              <a href={`${API}${file.url}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1 bg-white border border-gray-200 text-gray-700 font-bold uppercase tracking-widest text-[10px] rounded-md hover:bg-gray-100 transition-colors shadow-sm">
                                <FileText className="w-3 h-3" /> View Raw File
                              </a>
                            </div>
                          </div>
                          <button onClick={() => handleDeleteServerFile(file.name)} className="p-2.5 text-red-500 bg-white border border-red-200 hover:bg-red-50 rounded-xl transition-colors shadow-sm shrink-0" title="Delete Server Document">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      ))}
            ...
            {activeTab === 'late_authors' && <LateAuthorsSystemTab orders={orders} authors={authors} fetchAuthors={fetchAuthors} />}
            {activeTab === 'helpdesk' && <HelpdeskTab refreshTrigger={lastRefreshTime} />}
            {activeTab === 'event-requests' && <EventRequestsTab refreshTrigger={lastRefreshTime} />}
            {activeTab === 'library_donations' && <LibraryDonationsTab />}
            {activeTab === 'settings' && <SettingsTabComponent books={books} API={API} />}
          </Suspense>
        </div>
      </main>
...
<<<<<<< Updated upstream
                    <img loading="lazy" src={selectedBookDetails.coverUrl.startsWith('http') ? selectedBookDetails.coverUrl : `${API}${selectedBookDetails.coverUrl}`} alt="Cover" className="w-28 h-40 object-cover border border-paa-navy/20 shadow-sm rounded" />
=======
...
<<<<<<< Updated upstream
                    <img loading="lazy" src={selectedBookDetails.backCoverUrl.startsWith('http') ? selectedBookDetails.backCoverUrl : `${API}${selectedBookDetails.backCoverUrl}`} alt="Back Cover" className="w-28 h-40 object-cover border border-paa-navy/20 shadow-sm rounded" />
=======
...
>>>>>>> Stashed changes
      ...
      <Modal isOpen={isEditEventModalOpen} onClose={() => setIsEditEventModalOpen(false)} title="Edit Event" maxWidthClass="!max-w-4xl w-[90vw]">
        {editingEvent && (() => {
              try {
                const safeDate = editingEvent.date ? (!isNaN(new Date(editingEvent.date).getTime()) ? new Date(editingEvent.date).toISOString().substring(0, 10) : (typeof editingEvent.date === 'string' ? editingEvent.date.substring(0, 10) : '')) : '';
                const safeDays = (typeof editingEvent.duration === 'string' && editingEvent.duration.match(/(\d+)\s*Days?/i)) ? parseInt(editingEvent.duration.match(/(\d+)\s*Days?/i)[1]) : 0;
                const safeHours = (typeof editingEvent.duration === 'string' && editingEvent.duration.match(/(\d+)\s*Hours?/i)) ? parseInt(editingEvent.duration.match(/(\d+)\s*Hours?/i)[1]) : 0;
                ...
                return (
                  <form className="space-y-4" onSubmit={handleEditEventSubmit}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="dash-label">Category</label>
                        <select name="category" className="dash-input" value={editingEvent.category || ''} onChange={e => setEditingEvent({ ...editingEvent, category: e.target.value })}>
                          <option value="" disabled hidden>Select Category</option>
                          <option value="Housing Society">Housing Society</option>
                          <option value="College">College</option>
                          <option value="Book Fair">Book Fair</option>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          ...
                          <input required type="number" min="0" className="dash-input" value={safeDays} onChange={e => {
                            const h = (typeof editingEvent.duration === 'string' && editingEvent.duration.match(/(\d+)\s*Hours?/i)) ? parseInt(editingEvent.duration.match(/(\d+)\s*Hours?/i)[1]) : 0;
                            setEditingEvent({ ...editingEvent, duration: `${e.target.value} Days ${h} Hours` });
                          }} />
                        </div>
                        <div className="flex-1">
                          <label className="dash-label">No. of Hours</label>
                          <input required type="number" min="0" className="dash-input" value={safeHours} onChange={e => {
                            const d = (typeof editingEvent.duration === 'string' && editingEvent.duration.match(/(\d+)\s*Days?/i)) ? parseInt(editingEvent.duration.match(/(\d+)\s*Days?/i)[1]) : 0;
                ...
                return (
                  <div className="p-6 bg-red-50 text-red-600 rounded-lg">
                    ...
                    <button onClick={() => setIsEditEventModalOpen(false)} className="mt-4 px-4 py-2 bg-red-600 text-white rounded">Close</button>
                  </div>
                );
              }
            })()}
            ...
            <div className="p-8 border-b border-paa-navy/5 flex justify-between items-center bg-[#f8fafc]">
              ...
              <div className="flex gap-4 items-center">
                <button onClick={async () => {
                  ...
                  const workbook = new ExcelJS.Workbook();
                  const sheet = workbook.addWorksheet('Event Settlement Report');
                  
                  const baseHeaders = ['Author Name', 'Email', 'Phone', 'Registration Fee', 'Payment Paid', 'Transaction ID', 'Book Title', 'Category', 'MRP', 'Listed Stock', 'Sold Stock', 'Available Stock', 'Returned Stock', 'Revenue'];
                  const dateHeaders = uniqueDates.map((d: string, i: number) => `Day ${i + 1} Sold (${new Date(d).toLocaleDateString()})`);
                  ...
                  eventReportData.authors.forEach((author: any) => {
                    ...
                    if (author.books && author.books.length > 0) {
                      author.books.forEach((b: any) => {
                        const rowData = [author.name, author.email, author.phone, expectedFee, amountPaid, txnId, b.title, b.category, b.mrp, b.listedStock, b.soldStock, b.availableStock, b.returnedStock, b.revenue];
                        uniqueDates.forEach((d: string) => {
                          rowData.push(b.dayWiseSales ? b.dayWiseSales[d] || 0 : 0);
                        });
                        const addedRow = sheet.addRow(rowData);
                        addedRow.eachCell((cell) => cell.border = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } });
                      });
                    } else {
                      const rowData = [author.name, author.email, author.phone, expectedFee, amountPaid, txnId, "No Books Listed", "", "", "", "", "", "", ""];
      ...
      {viewingGalleryImage && (() => {
        const galleryImages = selectedGalleryEvent?.galleryEvent?.images
          ...
          : [];
        const currentIndex = galleryImages.findIndex((img: any) => `${API}${img.url}` === viewingGalleryImage);
          ...
          <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setViewingGalleryImage(null)}>
            ...
            {currentIndex > 0 && (
              <button 
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setViewingGalleryImage(`${API}${galleryImages[currentIndex - 1].url}`);
                }}
              >
...
<<<<<<< Updated upstream
            <img loading="lazy" src={viewingGalleryImage} className="max-w-full max-h-[90vh] object-contain rounded-lg shadow-2xl transition-transform" alt="Gallery Fullscreen" onClick={(e) => e.stopPropagation()} />
=======
            <img src={viewingGalleryImage} className="max-w-full max-h-[90vh] object-contain rounded-lg shadow-2xl transition-transform" alt="Gallery Fullscreen" onClick={(e) => e.stopPropagation()} loading="lazy" decoding="async" />
>>>>>>> Stashed changes
                      ...
                      {editingAuthor.books.map((b: any, bIdx: number) => {
                        ...
                        return (
                          <div key={b.id || bIdx} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-4">
                            <div className="flex gap-4">
<<<<<<< Updated upstream
                              {b.coverUrl && <img loading="lazy" src={import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL + b.coverUrl : "http://localhost:3001" + b.coverUrl} className="w-16 h-24 object-cover border border-gray-200" alt="Cover" />}
=======
...
const HelpdeskTab = ({ refreshTrigger }: any) => {
  ...
  const [filterType, setFilterType] = useState<'All' | 'Pending' | 'Answered' | 'Resolved' | 'Message' | 'Event Request'>('All');
  ...
  const fetchQueries = async () => {
    try {
      ...
      const mappedQueries = res.data
        .map((q: any) => ({
          ...q,
          itemType: q.message?.startsWith('[EVENT REQUEST]') || q.subject === 'Event Organization Request' ? 'Event Request' : q.subject?.startsWith('Contact Form') ? 'Message' : 'Query'
        }))
    ...
    <div className="space-y-6 w-full">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 border-b border-paa-navy/5 pb-4 gap-4">
        <div className="shrink-0">
          ...
          <p className="text-paa-gray-text text-sm hidden lg:block">Manage author queries and contact inquiries.</p>
        </div>
        <div className="flex flex-col lg:flex-row items-center gap-3 w-full lg:w-auto min-w-0 flex-1 justify-end">
          ...
          <div className="flex overflow-x-auto hide-scrollbar bg-gray-100 rounded-2xl p-1 w-full lg:w-auto gap-1 shrink-0 max-w-full">
            {[
              { id: 'All', label: 'All', color: 'bg-gray-800 text-white', count: queries.length },
              { id: 'Pending', label: 'Unopened', color: 'bg-orange-100 text-orange-800', count: queries.filter(q => q.itemType === 'Query' && q.status === 'Pending').length },
              { id: 'Answered', label: 'Opened', color: 'bg-[#ebd8c0] text-blue-800', count: queries.filter(q => q.itemType === 'Query' && q.status === 'Answered').length },
              { id: 'Resolved', label: 'Closed', color: 'bg-green-100 text-green-800', count: queries.filter(q => q.itemType === 'Query' && q.status === 'Resolved').length },
              { id: 'Message', label: 'Messages', color: 'bg-gray-800 text-white', count: queries.filter(q => q.itemType === 'Message').length }
            ].map(t => (
              <button
                key={t.id}
              ...
              >
                {t.label}
                <span className={`px-1.5 py-0.5 rounded-full text-[9px] min-w-[1.25rem] text-center leading-none ${filterType === t.id ? 'bg-white/30 text-current' : 'bg-gray-200 text-gray-600'}`}>{t.count}</span>
              </button>
            ))}
          </div>
          <button onClick={() => fetchQueries()} className="shrink-0 p-2 border border-paa-navy/20 bg-gray-50 hover:bg-gray-100 text-paa-navy shadow-premium hover:shadow-premium-hover transition-all duration-300 ease-out rounded-full active:scale-95 hover:-translate-y-0.5 hidden lg:block">
            <RefreshCw size={18} />
          </button>
        </div>
      </div>
          ...
          <div key={q.id} className="border border-gray-200 rounded-xl bg-white shadow-sm overflow-hidden transition-all duration-200 group">
            ...
            >
              <div className="flex items-center gap-4 flex-1">
                <div className={`w-1 h-10 rounded-full ${q.itemType === 'Event Request' ? 'bg-[#d4a017]' : q.itemType === 'Message' ? 'bg-blue-500' : q.status === 'Resolved' ? 'bg-green-500' : q.status === 'Pending' ? 'bg-orange-500' : 'bg-blue-500'}`}></div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${q.itemType === 'Event Request' ? 'bg-[#fffdf5] text-[#d4a017] border border-[#d4a017]' : q.itemType === 'Message' ? 'bg-[#ebd8c0] text-blue-800' : q.status === 'Resolved' ? 'bg-green-100 text-green-800' : q.status === 'Pending' ? 'bg-orange-100 text-orange-800' : 'bg-[#ebd8c0] text-blue-800'}`}>
                  ...
                  {(q.itemType !== 'Message' && q.itemType !== 'Event Request') && (
...
const parseEventMessage = (messageText: string) => {
  ...
  if (descIdx !== -1) {
    info.description = messageText.substring(descIdx + 12).replace(/\[STATUS:\s*\w+\]/g, '').trim();
  } else {
...
const EventRequestsTab = ({ refreshTrigger }: any) => {
  ...
  const handleAccept = async (id: string | number) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/admin/event-requests/${id}/accept`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleReject = async (id: string | number) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/admin/event-requests/${id}/reject`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
    ...
    <div className="space-y-6 w-full animate-fadeIn">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 border-b border-paa-navy/5 pb-4 gap-4">
        <div>
          ...
          <p className="text-paa-gray-text text-sm">Manage and review incoming organizer event requests.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 p-5 rounded-2xl shadow-md border border-blue-600/20 flex items-center justify-between text-white hover:scale-[1.02] transition-all duration-300">
          <div>
            <p className="text-xs text-blue-100 uppercase font-bold tracking-wider opacity-90">Total Requests</p>
            <h4 className="text-3xl font-black mt-1 tracking-tight">{queries.length}</h4>
          </div>
        ...
        <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-5 rounded-2xl shadow-md border border-amber-600/20 flex items-center justify-between text-white hover:scale-[1.02] transition-all duration-300">
          <div>
            <p className="text-xs text-amber-100 uppercase font-bold tracking-wider opacity-90">Pending</p>
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-green-600 p-5 rounded-2xl shadow-md border border-emerald-600/20 flex items-center justify-between text-white hover:scale-[1.02] transition-all duration-300">
          <div>
            <p className="text-xs text-emerald-100 uppercase font-bold tracking-wider opacity-90">Accepted</p>
            <h4 className="text-3xl font-black mt-1 tracking-tight">
        ...
        <div className="bg-gradient-to-br from-rose-500 to-red-600 p-5 rounded-2xl shadow-md border border-rose-600/20 flex items-center justify-between text-white hover:scale-[1.02] transition-all duration-300">
          <div>
            <p className="text-xs text-rose-100 uppercase font-bold tracking-wider opacity-90">Rejected</p>
            <h4 className="text-3xl font-black mt-1 tracking-tight">
        ...
        <table className="w-full border-collapse text-left text-sm">
          <thead>
            <tr className="bg-[#b44d28] text-white text-xs uppercase tracking-wider font-bold">
              <th className="px-4 py-3 border-b border-white/10 w-16 text-center">S.No</th>
              <th className="px-4 py-3 border-b border-white/10">Date Received</th>
              <th className="px-4 py-3 border-b border-white/10">Organizer</th>
              <th className="px-4 py-3 border-b border-white/10">Event Activities</th>
          ...
          <tbody>
            {loading ? (
              ...
              filteredQueries.map((q, idx) => (
                <tr 
                  key={q.id} 
                  className={idx % 2 === 0 ? 'bg-white hover:bg-gray-100 transition-all duration-200' : 'bg-[#ebd8c0] hover:bg-[#dfccb2] transition-all duration-200'}
                >
                  ...
                  <td className="px-4 py-3.5 border-b border-gray-100 align-middle">
                    <div className="font-semibold text-paa-navy">{q.author?.name || q.name || 'N/A'}</div>
                    <div className="text-xs text-gray-500">{q.author?.email || q.email || 'N/A'}</div>
                    {q.parsed?.phone && <div className="text-xs text-gray-400">{q.parsed.phone}</div>}
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto p-6 flex flex-col gap-4 relative">
            ...
            <h3 className="text-lg font-serif font-bold text-paa-navy border-b pb-2 flex items-center gap-2">
              <CalendarIcon className="text-paa-gold" size={20} /> Event Request Details
            </h3>
            
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Proposer Name</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.proposer || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Organisation Name</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.organisation || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Designation</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.designation || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Contact Person</p>
                <p className="font-semibold text-gray-800">{selectedRequest.author?.name || selectedRequest.name || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Contact Email</p>
                <p className="font-semibold text-gray-800">{selectedRequest.author?.email || selectedRequest.email || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Contact Phone</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.phone || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Event Activities</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.type || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-bold">Proposed Date / Time</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.proposedDate || 'N/A'} at {selectedRequest.parsed?.proposedTime || 'N/A'}</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-gray-400 uppercase font-bold">Location</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.location || 'N/A'}</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-gray-400 uppercase font-bold">Audience details</p>
                <p className="font-semibold text-gray-800">{selectedRequest.parsed?.audience || 'N/A'}</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-gray-400 uppercase font-bold">Event Description</p>
                <div className="bg-gray-50 p-3 rounded-lg border text-gray-700 mt-1 whitespace-pre-wrap">
                  {selectedRequest.parsed?.description || 'No description provided.'}
                </div>
              </div>
            </div>
